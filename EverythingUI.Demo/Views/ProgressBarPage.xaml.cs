...
public partial class ProgressBarPage : UserControl
{
    ...
    {
        if (!_isRunning && AnimatedProgressBar.Value < 100) { _timer?.Start(); _isRunning = true; }
    }

    private void PauseButton_Click(object sender, RoutedEventArgs e) { _timer?.Stop(); _isRunning = false; }

    private void ResetButton_Click(object sender, RoutedEventArgs e) { _timer?.Stop(); _isRunning = false; AnimatedProgressBar.Value = 0; }
}
